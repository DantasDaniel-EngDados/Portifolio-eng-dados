...
SQL_FILE_PATH = os.path.abspath(os.path.join(BASE_DIR, "../../sql/insert_temperatura_dados.sql"))
...
with DAG(
    ...
    def carregar(dados):
        ...
        cursor = conn.cursor()
        for row in dados:
            cursor.execute(insert_query, (row["cidade"], row["data"], row["temperatura"]))
        conn.commit()
        cursor.close()
        conn.close()
    ...
    trigger_a = TriggerDagRunOperator(
        task_id="trigger_a",
        trigger_dag_id="tabela_temperatura_media_diaria"
    )

    carregar(dados_diarios) >> trigger_a